    ...
    computeAABB(){
        ...
        for (var i = 0; i < this.fBuffer.length; i++) {
            ...
            for (var j = 0; j < vecT.length; j++) {
                // reassign min max values based on stored info from face buffer
                if (vecT[j] > this.maxXYZ[j]) {
                    this.maxXYZ[j] = vecT[j];
                }
                if (vecT[j] < this.minXYZ[j]) {
                    this.minXYZ[j] = vecT[j];
                }
            }
        }
    }
    ...
    */
    getAABB(minXYZ,maxXYZ){
        for (var i = 0; i < this.minXYZ.length; i++) {
            // assign values similar to constructor
